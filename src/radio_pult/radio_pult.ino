...
const int DATA_SIZE = JOYSTICK_COUNT+1; //(JOYSTICK_COUNT+1)/2+1;
...
void loop(){
  for (int i = 0; i < BUTTON_COUNT; i++) data[i] = analogRead(JOYSTICK_PINS[i]);
  int button = 0;
  for (int i = 0; i < BUTTON_COUNT; i++) button += int(!digitalRead(BUTTON_PINS[i]))<<i;
  data[DATA_SIZE-1] = button;
  radio.write(&data, DATA_SIZE); // sizeof(data)
}
